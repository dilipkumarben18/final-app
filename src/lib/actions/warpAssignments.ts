...
// releases the last saree (24th for Silk, 30th-40th for Tissue) that's
// been held on the machine into stock and marks the old warp fully
// completed.
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      if (!assignment.twentyFourthWovenAt) {
        throw new Error('The last saree has not been marked as woven yet.');
      }
      ...
      if (!assignment.nextAssignment || assignment.nextAssignment.status !== 'STARTED') {
        throw new Error('The new warp must be started before the last saree can be cut and received.');
      }
