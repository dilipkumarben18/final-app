...
import { getWarpCapacity } from '@/lib/warpCapacity';
...
const receiveSchema = z.object({
  ...
  // Upper bound is generous (Tissue warps run 30-40) — the real,
  // capacity-specific bound is enforced at runtime below since it depends
  // on which physical warp this assignment is on.
  sareeNumber: z.coerce.number().int().min(1).max(49),
});
...
export async function receiveSaree(input: unknown): Promise<ReceivingFormState> {
  ...
  try {
    const created = await prisma.$transaction(async (tx) => {
      ...
      const capacity = await getWarpCapacity(tx, data.warpAssignmentId);
      if (data.sareeNumber >= capacity) {
        throw new Error(`Saree ${capacity} is handled through the changeover step, not here.`);
      }
...
// The last saree (24th for Silk, 30th-40th for Tissue — see
// getWarpCapacity) is done on the machine but stays there — it is NOT
// added to stock here (spec section 9). This just records that fact and,
// if a next warp is already prepared, flips it to WAITING_TO_START.
export async function markTwentyFourthWoven(input: unknown): Promise<ReceivingFormState> {
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      if (assignment.twentyFourthWovenAt) throw new Error('The last saree is already marked woven.');
      const capacity = await getWarpCapacity(tx, assignment.id);
      if (assignment.receivingEntries.length < capacity - 1) {
        throw new Error(`Only ${assignment.receivingEntries.length}/${capacity - 1} sarees received so far.`);
      }
