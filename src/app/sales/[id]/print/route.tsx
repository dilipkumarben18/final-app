...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const sale = await prisma.sale.findUnique({
    where: { id: params.id },
    include: { firm: true, party: true, branch: true, items: { include: { sareeType: true } }, createdBy: true },
  });
  ...
  const received = await prisma.payment.aggregate({
    _sum: { amount: true },
    where: { saleId: sale.id, direction: 'RECEIVED' },
  });
  const paidAmount = Number(received._sum.amount ?? 0);
  const balance = Number(sale.totalAmount) - paidAmount;

  const subtotal = sale.items.reduce((sum, item) => sum + Number(item.amount), 0);
  const gstRatePercent = subtotal > 0 ? (Number(sale.gstAmount) / subtotal) * 100 : 0;
  const tax = computeTaxBreakdown(sale.firm.gstNumber, sale.party.gstNumber, subtotal, gstRatePercent);
  const placeOfSupply = stateFromGstin(sale.party.gstNumber);
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.firmBlock}>
          <Text style={styles.firmName}>{sale.firm.name}</Text>
          {sale.firm.address && <Text style={styles.firmLine}>{sale.firm.address}</Text>}
          {sale.firm.contactInfo && <Text style={styles.firmLine}>{sale.firm.contactInfo}</Text>}
          {sale.firm.gstNumber && (
            <Text style={styles.firmLine}>
              GSTIN: {sale.firm.gstNumber}
              {stateFromGstin(sale.firm.gstNumber) ? `, State: ${stateFromGstin(sale.firm.gstNumber)}` : ''}
            </Text>
          )}
        </View>
        ...
        <View style={styles.twoCol}>
          <View style={[styles.colBox, styles.colBoxLeft]}>
            ...
            <View style={styles.colBody}>
              <Text style={styles.bodyName}>{sale.party.name}</Text>
              {sale.branch && <Text style={styles.bodyLine}>Branch: {sale.branch.name}</Text>}
              {(sale.branch?.address || sale.party.address) && (
                <Text style={styles.bodyLine}>{sale.branch?.address || sale.party.address}</Text>
              )}
              {sale.party.gstNumber && <Text style={styles.bodyLine}>GSTIN: {sale.party.gstNumber}</Text>}
              {placeOfSupply && <Text style={styles.bodyLine}>State: {placeOfSupply}</Text>}
            </View>
          </View>
          <View style={styles.colBox}>
            <Text style={styles.colHeader}>Bill Details</Text>
            <View style={styles.colBody}>
              <Text style={styles.bodyLine}>Bill No.: {sale.saleNumber}</Text>
              <Text style={styles.bodyLine}>Invoice #: {sale.invoiceNumber}</Text>
              <Text style={styles.bodyLine}>Date: {formatBillDate(sale.date)}</Text>
              {placeOfSupply && <Text style={styles.bodyLine}>Place of supply: {placeOfSupply}</Text>}
              {sale.dueDate && <Text style={styles.bodyLine}>Due: {formatBillDate(sale.dueDate)}</Text>}
              {sale.ewayBillNumber && <Text style={styles.bodyLine}>E-Way Bill #: {sale.ewayBillNumber}</Text>}
              {sale.courierLrNumber && <Text style={styles.bodyLine}>Courier/LR #: {sale.courierLrNumber}</Text>}
            </View>
          </View>
        </View>
        ...
        <View style={styles.table}>
          ...
          {sale.items.map((item, i) => {
            ...
            return (
              <View key={item.id} style={styles.tr}>
                ...
                <Text style={[styles.td, styles.colItem]}>{item.sareeType.name}</Text>
                <Text style={[styles.td, styles.colHsn]}>{item.sareeType.hsnCode || '—'}</Text>
                <Text style={[styles.td, styles.colQty]}>{item.quantity.toString()}</Text>
                <Text style={[styles.td, styles.colUnit]}>Nos</Text>
          ...
          <View style={styles.totalRow}>
            ...
            <Text style={[styles.totalTd, styles.colGst]}>{formatNumber(Number(sale.gstAmount))}</Text>
            <Text style={[styles.totalTd, styles.colAmount]}>{formatNumber(Number(sale.totalAmount))}</Text>
          </View>
        </View>
        ...
        <View style={styles.twoCol}>
          <View style={[styles.colBox, styles.colBoxLeft]}>
            ...
            <View style={styles.colBody}>
              <Text style={styles.bodyLine}>{amountInWords(Number(sale.totalAmount))}</Text>
            </View>
          </View>
          <View style={styles.colBox}>
            ...
            <View style={styles.colBody}>
              ...
              {sale.courierCharges != null && (
                <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                  <Text style={styles.bodyLine}>Courier charges</Text>
                  <Text style={styles.bodyLine}>{formatCurrency(Number(sale.courierCharges))}</Text>
                </View>
              )}
              <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
              ...
              {sale.freightStatus && (
                <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                  <Text style={styles.bodyLine}>Freight</Text>
                  <Text style={styles.bodyLine}>{sale.freightStatus === 'PAID' ? 'Paid' : 'To Pay'}</Text>
                </View>
              )}
            </View>
          </View>
        ...
        {sale.notes && <Text style={[styles.bodyLine, { marginBottom: 10 }]}>Notes: {sale.notes}</Text>}
        ...
        <View style={styles.footerRow}>
          ...
          <View style={styles.footerCol}>
            <View style={styles.signBox}>
              <Text style={styles.bodyLine}>For : {sale.firm.name}</Text>
