...
type Weaver = {
  ...
  current: {
    ...
    capacity: number;
    alertAtRemaining: number;
  } | null;
  awaitingCut: { sareeTypeName: string; capacity: number } | null;
        ...
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            {weavers.map((w) => {
              ...
              const capacity = w.current?.capacity ?? 24;
              const alertAt = capacity - (w.current?.alertAtRemaining ?? 6);
              const alert = !!w.current && progress >= alertAt && progress < capacity;
              const canAssign = !w.pending && (!w.current || progress >= alertAt);
              return (
                  ...
                  <td className="px-4 py-3">
                    ...
                    {w.awaitingCut && (
                      <p className="mt-0.5 text-xs text-amber-700">
                        Previous {w.awaitingCut.sareeTypeName} warp: {w.awaitingCut.capacity}/{w.awaitingCut.capacity}{' '}
                        woven, awaiting cut on Saree Receiving.
                      </p>
                    )}
                  </td>
